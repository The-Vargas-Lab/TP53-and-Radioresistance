...
#Waterfall plot for Figure 2B with boxplots
JHLog <- read_xlsx("JHUEM1, 2 NTC + variants, Log2 AUC data, 11-30-21.xlsx", 1)
JHLog <- JHLog[,-1]
##Pivot data table and aggregrate with mean Log2 AUC/mCherry of each variant for each cell line
...
JHLog <- read_xlsx("JHUEM1, 2 NTC + variants, Log2 AUC data, 11-30-21.xlsx", 5)
...
##Pivot table to long form and convert KO column to factor
JHLog <- melt(JHLog, id= c("KO"))
JHLog$KO <-factor(JHLog$KO, levels= c("NTC", "KO 5.1C", "KO 5.1E", "KO 6.1B", "KO 6.1C"), 
       labels= c("Control", "5.1C", "5.1E", "6.1B", "6.1C"))

JSw <- ggplot(JHLog, aes(x= KO, y= value)) + 
  theme_classic() + scale_y_continuous(limits= c(-1, 0.2), breaks= seq(-1, 0.2, 0.5)) +
  labs(title= "JHUEM-2", x= NULL, y= "Log2 (WT Addback/mCherry)") +
  theme(axis.text.y= element_text(face= "bold", size= 16), axis.title.y= element_text(size= 18, face= "bold"), axis.text.x = element_text(face = "bold", size= 14), plot.title = element_text(size= 18, hjust= 0.5)) +
  theme(legend.position= "none", axis.line = element_line(size= 1)) 
JSw <- JSw + geom_boxplot(fill= "grey", width= 0.4)
JSw <- JSw + geom_beeswarm(aes(color= KO), priority = "density", cex= 3) + scale_color_manual(values = alpha(c("royalblue", "springgreen4","springgreen4", "firebrick2", "firebrick2"), 0.6))
JSw <- JSw + geom_hline(linetype= "dashed", yintercept = 0.00)

jpeg("JHUEM2 Boxplot, separate.jpeg", width= 1100, height= 1300, units= "px", res= 150)
print(JSw)

dev.off()


#Boxplots for JHUEM1 Figure 1E
JHLog <- read_xlsx("JHUEM1, 2 NTC + variants, Log2 AUC data, 11-30-21.xlsx", 4)
...
JSw <- ggplot(JHLog, aes(x= KO, y= value)) + 
  theme_classic() + scale_y_continuous(limits= c(-1, 0.4), breaks= seq(-1, 0.4, 0.5)) +
  scale_x_discrete(expand= expansion(mult= c(0.25, 1))) + labs(title= "JHUEM-1", x= NULL, y= "Log2 (WT Addback/mCherry)") +
  theme(axis.text.y= element_text(face= "bold", size= 16), axis.title.y= element_text(size= 18, face= "bold"), axis.text.x = element_text(face = "bold", size= 14), plot.title = element_text(size= 18, hjust= 0.5)) +
  theme(legend.position= "none", axis.line = element_line(size= 1))
JSw <- JSw + geom_boxplot(fill= "grey", width= 0.4)
JSw <- JSw + geom_beeswarm(aes(color= KO), priority = "density", cex= 3) + scale_color_manual(values = alpha(c("royalblue", "springgreen4", "firebrick2"), 0.6))
...
jpeg("Figure 1E JHUEM1 Boxplot.jpeg", width= 1100, height= 1300, units= "px", res= 150)
print(JSw)
...
#Boxplots for JHUEM1 Figure 1E
JHLog <- read_xlsx("JHUEM1, 2 NTC + variants, Log2 AUC data, 11-30-21.xlsx", 6)
...
##Pivot table to long form and convert KO column to factor
JHLog <- melt(JHLog, id= c("KO"))
JHLog$KO <-factor(JHLog$KO, levels= c("NTC", "KO 5.1C", "KO 5.1L", "KO 6.1J", "KO 6.1L"), 
                  labels= c("Control", "5.1C", "5.1L", "6.1J", "6.1L"))

JSw <- ggplot(JHLog, aes(x= KO, y= value)) + 
  theme_classic() + scale_y_continuous(limits= c(-1, 0.4), breaks= seq(-1, 0.4, 0.5)) +
  labs(title= "JHUEM-1", x= NULL, y= "Log2 (WT Addback/mCherry)") +
  theme(axis.text.y= element_text(face= "bold", size= 16), axis.title.y= element_text(size= 18, face= "bold"), axis.text.x = element_text(face = "bold", size= 14), plot.title = element_text(size= 18, hjust= 0.5)) +
  theme(legend.position= "none", axis.line = element_line(size= 1))
JSw <- JSw + geom_boxplot(fill= "grey", width= 0.4)
JSw <- JSw + geom_beeswarm(aes(color= KO), priority = "density", cex= 3) + scale_color_manual(values = alpha(c("royalblue", "springgreen4","springgreen4", "firebrick2", "firebrick2"), 0.6))
...
jpeg("JHUEM1 Boxplot, separate.jpeg", width= 1100, height= 1300, units= "px", res= 150)
print(JSw)
